...
import psycopg2
from babel_spider.items import *
...
class BabelSpiderPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            if type(item) is BabelSpiderFeedItem:
                sql="""
                        INSERT INTO articles (url, category_id, media_id, published_at) VALUES(%s, %s, %s, %s)
                    """
                self.cursor.execute(sql, (item.get('url'), item.get('category_id'), item.get('media_id'), item.get('published_at'),))
